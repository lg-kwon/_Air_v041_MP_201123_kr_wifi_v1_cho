...
#ifdef  INCLUDE_KEY_CONTROL
    if(GPIO_ReadInputDataBit(GPIOE, GPIO_Pin_2) == RESET)		flag |= KEY_SW1_CHECK;
    if(GPIO_ReadInputDataBit(GPIOE, GPIO_Pin_3) == RESET)		flag |= KEY_SW2_CHECK;
    if(GPIO_ReadInputDataBit(GPIOE, GPIO_Pin_4) == RESET)		flag |= KEY_SW3_CHECK;
    if(GPIO_ReadInputDataBit(GPIOE, GPIO_Pin_5) == RESET)		flag |= KEY_SW4_CHECK;
    if(GPIO_ReadInputDataBit(GPIOE, GPIO_Pin_6) == RESET)		flag |= KEY_SW5_CHECK;
#else
...
#ifdef  INCLUDE_KEY_CONTROL
  if(type == TYPE_NORMAL) {
    if (flag&KEY_SW1_CHECK) g_keyFlag |= KEY_SW1_FLAG;
    if (flag&KEY_SW2_CHECK) g_keyFlag |= KEY_SW2_FLAG;
    if (flag&KEY_SW3_CHECK) g_keyFlag |= KEY_SW3_FLAG;
    if (flag&KEY_SW4_CHECK) g_keyFlag |= KEY_SW4_FLAG;
    if (flag&KEY_SW5_CHECK) g_keyFlag |= KEY_SW5_FLAG;
  }
  else if(type == TYPE_LONG) {
    if (flag&KEY_SW1_CHECK) g_keyFlag |= KEY_SW1_LONG_FLAG;
    if (flag&KEY_SW2_CHECK) g_keyFlag |= KEY_SW2_LONG_FLAG;
    if (flag&KEY_SW3_CHECK) g_keyFlag |= KEY_SW3_LONG_FLAG;
    if (flag&KEY_SW4_CHECK) g_keyFlag |= KEY_SW4_LONG_FLAG;
    if (flag&KEY_SW5_CHECK) g_keyFlag |= KEY_SW5_LONG_FLAG;
  }/*
